...
protected:
    /**
     * This constructor takes a MouseEventable subclass, so that we can register the functions that may or may not have been overridden
    ...
    MouseEventable(T child){
        // enforce that we can only construct with MouseEventable types
