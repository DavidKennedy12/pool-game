...
public:
    ...
    /**
     * @brief clone - return a deep copy of the game
     */
    virtual Game* clone () const;

    /**
     * @brief saveToMemento - create a memento of current game state
     */
    Memento* saveToMemento();


    /**
     * @brief restorFromMemento - restor the game state from a saved memento
     * @param memento - the memento to be restored
     */
    void restoreFromMemento(Memento memento);


    /**
     * @brief readyToShoot - checks if game is ready for next shot to be played
     * @return - true if all balls are stationary
     */
    bool readyToShoot();

    /**
     * @brief applyMove - apply a shot to the cue ball
     * @param move - the velocity to be imparted on the cue ball
     */
    void applyMove(QVector2D move);

    bool m_myTurn = true;
    int m_redBallsSunk = 0;
    int m_yellowBallsSunk = 0;
